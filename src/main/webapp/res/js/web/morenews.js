...
function addGra(){	
    ...
    var hiddenDiv = "<input type='hidden' id='left_title_"+hiddenCount+"' >"
					...
					+"<input type='hidden' id='left_show_cover_pic_"+hiddenCount+"' value=0>"
...
function fnClick(obj){
	...
	//作者
	var leftAuthor = $("#now_div").find("#left_author_" + divIndex).val();
...
function clearInputDiv(){
	...
	//显示方面图片地址
	var imgShowURL = $("#fileImage1").attr("src");
	if(imgShowURL){
		$("#fileImage1").attr("src","");
		$("#fileImage1").hide();
	}
...
function checkMoreNews(){
	...
	if(divLength < 2){
		...
		for(var i=0;i<divLength;i++){
			...
			//封面图片id
			var leftThumbMediaId = $(newDiv).find("#left_thumb_media_id_" + i).val().trim();
			...
			//原文链接
			var leftContentSourceUrl = $(newDiv).find("#left_content_source_url_" + i).val().trim();
	        ...
	        //图片地址
			var picPathValue = $(newDiv).find("#left_mainImage_" +  i).val();
