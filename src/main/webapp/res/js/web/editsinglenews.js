$(document).ready(function() { 
	...
	$("#fileImage1").hide();
	var showPicValue = $("#left_show_cover_pic").val();
	...
	//图片路径
	var imgPath =  $("#left_mainImage").val();
	//alert(imgPath);
	if(imgPath){
		$("#fileImage1").attr("src",imgPath);
		//左侧图片显示
		$(".graBig").find("img").attr("src",imgPath);
		$("#fileImage1").show();
	}else{
		$("#fileImage1").hide();
	}
...
function checkSingleNews(){
	...
	//内容
	var leftContent = $("#left_content").val().trim();
	
	var leftContentValue = $("#left_content").val();
	if(leftContentValue == ""){
		UE.getEditor('content').focus();
		$("#content_tip").html("内容不能为空");
		return false;
	...
	articleObj.id = $("#newsId").val();
	articleObj.mediaId = $("#mediaId").val();
...
function doSaveSingleNews(articleObj){
	...
	var result = "";
	$.ajax({
		url:path + '/msgnews/updateSingleNews',
	    type:'POST',
	    async: false,
	    dataType:'json',
	    data:articleObj,
