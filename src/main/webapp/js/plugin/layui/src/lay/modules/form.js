...
layui.define('layer', function(exports){
  ...
  Form.prototype.on = function(events, callback){
    return layui.onevent.call(this, MOD_NAME, events, callback);
      ...
      //添加表单验证事件  add by hermit
    Form.prototype.validate = function(formId){
        if(formId){
            return submit(formId);
        }else{
            return submit('add_form');
        }
    };
  ...
  var submit = function(formId){
          //添加formId参数 add by hermit
      var formElem,verifyElem,fieldElem,filter;
    ...
    layui.each(verifyElem, function(_, item){
      ...
      ,value = othis.val();
      //add by zhaoqf 非必填为空返回,
      if(!ifinArray(vers,"required") && !verify["required"][0].test(value)){
        return true;
      }
      ...
      layui.each(vers, function(_, thisVer){
        ...
        //匹配验证规则
        if(verify[thisVer]){
          //add by zhaoqf,解决null过必填的bug
          var isTrue = isFn ? errorText = verify[thisVer](value, item) : !verify[thisVer][0].test(value==null?"":value);
          ...
          if(isTrue){
            ...
            if(verType === 'tips'){
              ...
              //修改提示样式 add by hermit
                layer.msg(errorText, {icon: 5, shift: 0});
            }
    ...
    layui.each(fieldElem, function(_, item){
      ...
      if((item.type).indexOf("multiple") != -1) {
          ...
          // 原：field[item.name] = item.value
          //add by zhaoqf checkbox会拼成name[v,v,v]
          if(item.type=="checkbox"){
              field[item.name]?field[item.name].push(item.value):(field[item.name]=[item.value]);
          }else{
