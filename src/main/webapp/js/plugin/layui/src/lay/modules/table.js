...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,table = {
    config: {
      checkName: 'checked' //是否选中状态的字段名 add by hermit
      ,indexName: 'LAY_TABLE_INDEX' //下标索引名
      
      //修改默认参数 add by hermit
        ,request:{
            pageName: 'page'//page
          ...
          ,'<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
            ,'<div class="layui-table-cell laytable-cell-'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# } }}'
              ,'{{# } }}'
            ,'" align="center">'//修改表头标题居中显示 add by hermit
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    ,sort = function(){
      if(typeof options.initSort === 'object'){
        ...
        //自定义函数，分页相关 add by hermit
    ,getPageTotal=function (res) {
        return res["page"]?res["page"][response.countName]:1;
    },setPageTotal=function (val) {
        res["page"] && (res["page"][response.countName]=val);
    };
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          ...
          typeof options.done === 'function' && options.done(res, curr, getPageTotal(res));
        }
        ,error: function(e, m){
      ...
      setPageTotal(options.data.length);
  ...
  Class.prototype.events = function(){
    ...
    //添加行点击事件 add by hermit
      that.layBody.on('click', 'tr', function(){
          var othis = $(this)
              ,index = othis.index()
              ,data = table.cache[that.key][index];

          layui.event.call(this, MOD_NAME, 'clickRow('+ filter +')', {
              index: index
              ,data: data
          });
      });
